...
load('Suppl_Figure4.mat')

% Suppl_Figure4a Left - violin plots of differences between PRE, STIM, POST
...
% Suppl_Figure4a Right - topographies of differences between PRE, STIM, POST
...
% Suppl_Figure3b Left - topographies of differences between EARLY and LATE stim
% for TES
figure(5); clf; set(gcf, 'Position', [80 307 837 591], 'Visible', 'on')
hold on
...
% Suppl_Figure3b Right - violin plots of differences between EARLY and LATE stim
% for TES
% early_v_late_stim values are normalized SWA (dim: subj x 1)
figure(6); clf; set(gcf, 'Position', [100 357 240 498], 'Visible', 'on')
hold on
all_d = [early_v_late_tes_stim*100];
all_d = all_d(:);
all_lab = [repmat({'a'},length(early_v_late_tes_stim),1)];
all_lab = all_lab(:);
vs = violinplot(all_d, all_lab);
for i = 1
